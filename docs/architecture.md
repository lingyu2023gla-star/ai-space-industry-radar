...
flowchart TD
    A[ask] --> B[retriever]
    B --> C[citation builder]
    C --> D[local answer]
    C --> E[LLM prompt]
    D --> F["answer with [1][2] evidence"]
    E --> F
```
...
```mermaid
flowchart TD
    A[research command] --> B[retriever]
    B --> C[research_session.py]
    C --> D[local notes]
    C --> E[optional LLM analysis]
    D --> F[outputs/research_session.md]
    E --> F
    F --> G[optional report_ingestor]
    G --> H[storage]
    H --> I[ask]
```
